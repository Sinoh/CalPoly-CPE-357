...
int check_command_line(char *command_line) {
  ...
  int argc, input_flag, input_flag_arg, output_flag, output_flag_arg, pipes,
      pipe_input, pipe_output, pipe_flag_arg;
  char *token;

  argc = input_flag = input_flag_arg = output_flag = output_flag_arg = pipes =
      pipe_input = pipe_output = pipe_flag_arg = 0;
...
void run_command(struct Command *cmd, int stage_number) {
  int stages, pipes, i, j, in_fd, out_fd, fd[PIPE_LIMIT * 2];
  ...
  if ((strcmp(temp->input, "original stdin")) &&
      (strcmp(temp->input, "pipe from stage 0"))) {
...
int parse_line(char *command_line, struct Command *cmd, int stage_number) {
  ...
  if ((strlen(token) > 0) && (command_line[strlen(token) - 1] == '\n')) {
    command_line[strlen(token) - 1] = '\0';
  }
...
/* Frees every part of the Command Struct */
